...
var Group = function () {
  ...
  _createClass(Group, null, [{
    ...
    value: function create(state) {
      ...
      state = state.setIn(['scene', 'groups', groupID], new _models.Group({ id: groupID, name: groupID }));
      ...
      layerList.entrySeq().forEach(function (_ref7) {
        ...
        if (lines) {
          ...
          for (var vertexID in vertices) {
            ...
            state = _export.Vertex.setAttributes(state, groupLayerID, vertexID, new _immutable.Map({ x: xV + deltaX, y: yV + deltaY })).updatedState;
          }
        ...
        if (items) state = items.map(function (itemID) {
          ...
          return _export.Item.setAttributes(newState, groupLayerID, item.id, new _immutable.Map({ x: xI + deltaX, y: yI + deltaY })).updatedState;
        }, state);
    ...
    value: function rotate(state, groupID, newAlpha) {
      ...
      state = Group.setAttributes(state, groupID, new _immutable.Map({ rotation: newAlpha })).updatedState;
      ...
      layerList.entrySeq().forEach(function (_ref9) {
        ...
        if (lines) {
          ...
          for (var vertexID in vertices) {
            ...
            state = _export.Vertex.setAttributes(state, groupLayerID, vertexID, new _immutable.Map({ x: newX, y: newY })).updatedState;
          }
          //need to be separated from setAttributes cycle
        ...
        if (items) state = items.map(function (itemID) {
          ...
          return _export.Item.setAttributes(newState, groupLayerID, item.id, new _immutable.Map({ x: newX, y: newY, rotation: rI + alpha })).updatedState;
        }, state);
