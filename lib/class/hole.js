  ...
  _createClass(Hole, null, [{
    ...
    value: function updateDrawingHole(state, layerID, x, y) {
      ...
      if (snap) {
        ...
        var minVertex = _export2.GeometryUtils.minVertex({ x: x1, y: y1 }, { x: x2, y: y2 });
        var maxVertex = _export2.GeometryUtils.maxVertex({ x: x1, y: y1 }, { x: x2, y: y2 });
    ...
    value: function setAttributes(state, layerID, holeID, holesAttributes) {
      ...
      state = state.mergeIn(['scene', 'layers', layerID, 'holes', holeID], (0, _immutable.fromJS)(hAttr)).mergeDeepIn(['scene', 'layers', layerID, 'holes', holeID], new _immutable.Map({ offset: offset, misc: misc }));
